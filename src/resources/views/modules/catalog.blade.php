...
@section('content')
    <div class="mb-4">
        ...
        <p class="text-muted mb-0">Activez ou désactivez les modules de votre entreprise. La désactivation conserve vos données.</p>
    </div>
    ...
    @foreach($modules as $group => $items)
        <h2 class="h6 text-uppercase text-muted mt-4 mb-3">{{ $group === 'Socle' ? 'Modules de base' : 'Modules métiers' }}</h2>
        <div class="row g-3">
            @foreach($items as $m)
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0 shadow-sm h-100" style="{{ $m['active'] ? 'box-shadow:0 0 0 2px #19875440 !important;' : '' }}">
                        <div class="card-body d-flex flex-column">
                            <div class="d-flex align-items-center gap-2 mb-2">
                                ...
                                <div>
                                    ...
                                    @if($m['sector'])<div class="text-muted small">{{ $m['sector'] }}</div>@endif
                                </div>
                            </div>
